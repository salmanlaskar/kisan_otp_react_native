...
import {StyleSheet, Text, View, RefreshControl,Dimensions, SectionList, TouchableOpacity} from 'react-native';
...
const ContactScreen = ({navigation}) => {
  ...
  const onChange = ({ window}) => {
    setHeight(window.height);setWidth(window.width);
  };
 const getData = () => {
    let contactsArr = [];
    ...
    <View style={[styles.container]}>
        <SectionList
          ...
          ListHeaderComponent={() => {return (
              <Text style={styles.headerText}>Total contacts: {contactlist.length}</Text>
          );}}
          renderItem={({ item }) => (
